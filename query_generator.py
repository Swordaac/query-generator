...
class operation:
    ...
    def __init__(self, df_name, leading, count=None):
        '''
...
class selection(operation):
    ...
    def __init__(self, df_name, conditions, count=None, leading=True):
    ...
    def to_str(self, df2="F"):
        ...
        if len(self.conditions) == 1:
            if df2.__eq__("F"):
                ...
                cur_condition = cur_condition + "df" + df2 + "[" + "'" + self.conditions[0].col + "'" + "]" + " " + \
                                self.conditions[0].op.value + " " + str(self.conditions[0].val)
        ...
        for i, condition in enumerate(self.conditions):
            ...
            if type(condition) == OP_cond:
                ...
                if df2.__eq__("F"):
                    ...
                    cur_condition += "(" + "df" + df2 + "[" + "'" + cond.col + "'" + "]" + " " + cond.op.value + " " + str(
                        cond.val) + ")"
...
class merge(operation):
    ...
    def __init__(self, df_name, queries: 'pandas_query', count=None, on=None, left_on: str = None, right_on: str = None,
                 leading=False):
        '''

        :param df_name: name of the dataframes
        :param queries: the following queries after the merge   q2里的query
...
class projection(operation):
    def __init__(self, df_name, columns: List[str], count=None, leading=True):
        '''
...
class group_by(operation):
    def __init__(self, df_name, columns, count=None, other_args=None, leading=False):
        '''
...
class agg(operation):
    def __init__(self, df_name, dict_columns: Union[str, Dict[str, str]], count=None, leading=True):
        """
...
class pandas_query():
    ...
    List[pandas_query] is the list of operations of this query
    '''
    ...
    def __init__(self, q_gen_query: List[operation], source: 'TBL_source', verbose=False):
        ...
        self.pre_gen_query = self.setup_query(q_gen_query)  # 一个东西，只是一定可以run
    ...
    def possible_selections(self, length=50):
        ...
        :param length: a threshold that prevents extra time consumption 最多扩容50个
        ...
        for key in possible_selection_columns:  # for example: key is col1, which is int datatype
            possible_condition_columns[key] = []  # key 是col name，value是condition object list
            description = self.get_source_description(source_df, key)  # ???
    ...
    def setup_query(self, list_op: List[operation]):
        '''
        test if the list_op works for this query, if not then change it to a working format

        :param list_op: check if the columns will be changed after the operations
        :return: a list of operations that functions properly
        ...
        for i, operation_ in enumerate(list_operation):  # input list，i是index，operation_ 是object
            if isinstance(operation_, projection):
                source_cols = operation_.desire_columns[:]  # desire_col是 col to be projected
                changed = True
            elif isinstance(operation_, group_by):  # 可能有其他的判断，再看一下merge的debug（比如project掉了col，之后却还是需要merge）
                ...
                if isinstance(operation_, group_by) and changed:
                    ...
                    for g_col in operation_.columns:
                        if g_col not in source_cols:
                            ...
                            operation_.set_columns([col])  # only one group_by column from source column
...
扩容生成list of query
        ...
        First, use set up query to make sure the query works well, such as project only on the existing columns, rather than having
        two projections that don't work
        Second, use gen query to generate a list of base query, and each query is actually a list of operations.
        '''
        generated_queries = []
        for operation in self.pre_gen_query:  # a list of operations

            possible_new_operations = []  # possible operations for one categopry of operation，对于每一个operation扩容成一个同类型的list
    ...
    def get_new_pandas_queries(self, out=1000):
        ...
        :return: list of pandas queries
        '''
        res = []
        new_queries = self.gen_queries()

        random.shuffle(new_queries)  # 先shuffle，避免只截取了col1的condition
    ...
    def generate_possible_column_combinations(self, operation: projection, generate_num=200):
        columns = self.get_source().columns  # 本质上不应该用source，而是到目前为止的df的现存col，可是memory占用太多了
        possible_columns = []
        ...
        if len(columns) == 1:
            ...
            res = [list(i) for i in
                   list(itertools.combinations(columns, operation.length))]  # 比如7选3，output所有的combination
...
merge然后再扩容
    ...
    # def unravel(self,query:pandas_query):
    #     count=0
    #     for q in self.result_queries:
    #         for s in q.pre_gen_query:
    #
    #             strs=s.to_str()
    #             if isinstance(s,merge):
    #                 for other in s.queries.pre_gen_query:
    #                     b=max(self.unravel(other))
    #                     print(f"df{b} = {strs} \n")
    #                     count+=1
    #                     return count
    #
    ...
    def save_merged_examples(self, dir, filename):
        ...
        #             current = q.pop(0)
        #         # do smth to merge
        #             for ops in current.queries.pre_gen_query:
        #                 ops.set_count(count)
        #
        #                 res[current.count].append(count)
        #                 count += 1
        #
        #
        #                 if ops is merge:
        ...
        def unravel(query: pandas_query):
            ...
            for s in query.pre_gen_query:
                ...
                if isinstance(s, merge):
                    ...
                    print(f"df{self.count} = df{counter}.merge(df{self.count - 1}, left_on={s.left_on}, right_on={s.right_on}) \n")
                    self.count += 1
                    continue

                #

                #print(f"df{self.count} = {strs} \n")



                if s.leading:
                    s1 = s.to_str()
                    print(f"df{self.count} = {s1} \n")  # strs
                    ...
                    if isinstance(s, selection):
                        ...
                        s2 = s.to_str()
                    print(f'df{self.count} = df{self.count - 1}' + s2)  # q.pre_gen[i].to_str(count-1)
                self.count += 1
        ...
        # for q in self.result_queries:
        #     # strs = q.
        #     for op in q.pre_gen_query:
        #         if not isinstance(op,merge):
        #             f.write(f"df{count} = {strs} \n")
        #             count +=1
        #         else:
        #             save_merged_examples(self, dir, filename)
        #     strs = q.query_string
        #     f.write(f"df{count} = {strs} \n")
        #     count += 1
    ...
    def save_unmerged_examples(self, dir, filename):
        ...
        for q in self.un_merged_queries:
            # strs = q.
            strs = q.query_string
            # what is query string==panadas query format
            try:
                p = eval(q.query_string)
            except Exception:
                print("%%%%%%%%%%% An Unexpected Exception has occured %%%%%%%%%%%%%%%%")
            # op1 = q.pre_gen_query[0]
            # if not(isinstance(op1,selection)):
            #     continue#the first is always selection operation
            for i in range(len(q.pre_gen_query)):
                ...
                if i == 0:
                    ...
                    f.write(f'df{count} = df{count - 1}' + s2)  # q.pre_gen[i].to_str(count-1)
                    count += 1
    ...
    def check_merge_left_right(self, q1: pandas_query, q2: pandas_query):
        ...
        if "series" in str(type(q1.get_target())) or "series" in str(type(q2.get_target())):  # what is this for? ***
            return []
    ...
    def generate_possible_merge_operations(self, max_merge=3, max_q=100):
        cur_queries = self.queries[:]
        random.shuffle(cur_queries)  # why??? ***
        k = 0
        res_hash = {}
        q_generated = 0
        while True:
            if k >= max_merge:  # 最多三个merge
                break
            for i in tqdm(range(len(cur_queries) - 1)):  # tqdm是进度条
                for j in range(i + 1, len(cur_queries)):  # i，j是两个不同的query，进行比较

                    if q_generated > max_q:  # how is it different from k
                        break

                    if str(i) + "+" + str(j) not in res_hash:  # res_hash存进去已经merge的index pair，如果不重复的话再进行下次merge
                        ...
                        merge_differenet_keys = self.check_merge_left_right(q1,
                                                                            q2)  # check if the forgein key is still there
                        ...
                        if len(merge_differenet_keys) > 0:
                            ...
                            operations.append(merge(df_name=q1.df_name, queries=q2, left_on=merge_differenet_keys[0],
                                                    right_on=merge_differenet_keys[
                                                        1]))  # append merge,然后list（q1, merge(q2))
                            ...
                            ###################################################
                            cols = self.check_merge_on(q1, q2)  # what is this for? ***

                            if cols and max(q1.num_merges, q2.num_merges) < 3 and self.self_join:
                                # print(cols)
                                operations = list(q1.pre_gen_query)[:]
                               # operations2 = list(q2.pre_gen_query[:])
...
class TBL_source():
    ...
    def gen_base_queries(self) -> List[pandas_query]:
        ...
        # queries = []
        q1 = pandas_query(q_gen_query=[self.get_a_selection(), self.get_a_projection(), self.get_a_aggregation()],
                          source=self)
        q2 = pandas_query(q_gen_query=[self.get_a_selection(), self.get_a_projection(), self.get_a_selection()],
                          source=self)
...
if __name__ == "__main__":
    # run_TPCH()
    customer = pd.read_csv(os.path.expanduser("/Users/huohongxin/Downloads/benchmarks/customer_2.csv"))
    lineitem = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/lineitem_2.csv")
    nation = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/nation_2.csv")
    orders = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/orders_2.csv")
    part = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/part_2.csv")
    partsupp = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/partsupp_2.csv")
    region = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/region_2.csv")
    supplier = pd.read_csv("/Users/huohongxin/Downloads/benchmarks/supplier_2.csv")
    print(customer)
    ...
    # pandas_queries_list.unravel(pandas_queries_list.result_queries)
